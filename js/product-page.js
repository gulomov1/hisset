document.addEventListener("DOMContentLoaded",()=>{
  ...
  function setStatus(text,error=false){status.textContent=text;status.style.color=error?"#9d214d":"";status.style.background=error?"#fff0f5":""}
  ...
  function showStep(number,focus=true){closeSelect();state.step=number;steps.forEach(step=>step.hidden=Number(step.dataset.step)!==number);form.querySelectorAll("[data-progress-step]").forEach(label=>{if(Number(label.dataset.progressStep)===number)label.setAttribute("aria-current","step");else label.removeAttribute("aria-current")});backButton.hidden=number!==2;setRetry(state.retry);updateButton();setStatus(number===1?"Maʼlumotlarni kiriting va davom eting.":"Yetkazib berish joyini tanlang.");modal.querySelector(".modal_inner").scrollTop=0;if(focus)steps[number-1].querySelector(".step_title").focus();if(number===2)loadConfig()}
  ...
  form?.addEventListener("input",event=>{if(!modalOpen()||state.busy)return;if(event.target.name==="phone")event.target.value=formatLocalPhone(event.target.value);event.target.setCustomValidity?.("");updateButton();if(state.step===1)setStatus("Maʼlumotlarni kiriting va davom eting.")});
